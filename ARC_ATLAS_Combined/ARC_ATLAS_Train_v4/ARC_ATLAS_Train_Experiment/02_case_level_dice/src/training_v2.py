...
class WholeBrainValidationCallback(tf.keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if case_soft:
            _case_soft = np.asarray(case_soft, dtype=np.float32)
            logs["val_case_dice_p25"] = float(np.percentile(_case_soft, 25.0))
            logs["val_case_dice_p50"] = float(np.percentile(_case_soft, 50.0))
            logs["val_case_dice_p75"] = float(np.percentile(_case_soft, 75.0))
        ...
        if source_soft:
            ...
            if case_soft:
                logger.info("Whole-brain val case Dice percentiles: p25=%.5f p50=%.5f p75=%.5f", logs["val_case_dice_p25"], logs["val_case_dice_p50"], logs["val_case_dice_p75"])
