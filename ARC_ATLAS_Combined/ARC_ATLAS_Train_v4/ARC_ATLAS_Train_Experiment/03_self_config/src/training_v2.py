...
class WholeBrainValidationCallback(tf.keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        for i, (img_p, msk_p) in enumerate(eval_pairs, start=1):
            ...
            src_name = Path(str(img_p)).name
            src_name = src_name.split("__", 1)[0] if "__" in src_name else src_name.split("_", 1)[0]
...
def _apply_self_config(config: DynamicTrainingConfig, num_pairs: int) -> DynamicTrainingConfig:
    """Small-dataset heuristics to avoid over-provisioned training loops."""
    if not bool(getattr(config, "AUTO_SELF_CONFIG", False)):
        return config
    n = max(1, int(num_pairs))
    if n <= 16:
        config.EPOCH_STEPS = min(int(config.EPOCH_STEPS), max(12, n * 6))
        config.TOTAL_EPOCHS = min(int(config.TOTAL_EPOCHS), 60)
        config.DIFF_AWARE_ENABLED = False
    elif n <= 64:
        config.EPOCH_STEPS = min(int(config.EPOCH_STEPS), max(40, n * 4))
    config._write_config()
    logger.info("AUTO_SELF_CONFIG enabled: pairs=%d, epoch_steps=%d, total_epochs=%d", n, int(config.EPOCH_STEPS), int(config.TOTAL_EPOCHS))
    return config
def train_dynamic_model(config: Optional[DynamicTrainingConfig] = None, **overrides):
    ...
    config = _apply_self_config(config, len(pairs))
