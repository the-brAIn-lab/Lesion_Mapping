...
class DynamicTrainingConfig:
    ...
    HARMONIZE_INTENSITY: bool = False
...
def _robust_intensity_harmonize(volume: np.ndarray) -> np.ndarray:
    vol = np.nan_to_num(volume, nan=0.0, posinf=0.0, neginf=0.0).astype(np.float32, copy=False)
    nz_mask = vol > 0
    if int(np.sum(nz_mask)) < 64:
        return vol
    nz = vol[nz_mask]
    ...
    vol = (vol + 5.0) / 10.0
    vol = vol * nz_mask.astype(np.float32)
...
def _load_and_preprocess_image(path: str, target_shape: tuple[int, int, int] | None) -> np.ndarray:
    ...
    cfg = globals().get("_ACTIVE_CONFIG")
    if bool(getattr(cfg, "HARMONIZE_INTENSITY", False)):
        vol = _robust_intensity_harmonize(vol)
