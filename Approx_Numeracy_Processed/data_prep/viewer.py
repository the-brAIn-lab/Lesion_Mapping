...
def _pairs_dirs(images_dir: Path, masks_dir: Path) -> dict:
    """Build pairs from two flat directories: one for T1s, one for masks."""
    pairs = {}
    for t1 in sorted(images_dir.glob("*.nii.gz")):
        mask_name = t1.name.replace("_T1w_MNI_norm", "_lesion_mask_MNI_clean")
        mask = masks_dir / mask_name
        if not mask.exists():
            continue
        label = t1.name.replace("_T1w_MNI_norm.nii.gz", "")
        idx = 2
        lbl = label
        while lbl in pairs:
            lbl = f"{label} ({idx})"; idx += 1
        pairs[lbl] = {"t1": t1, "mask": mask}
    if not pairs:
        raise RuntimeError(f"No matched T1/mask pairs found.\n  images: {images_dir}\n  masks:  {masks_dir}")
    return pairs


def show_viewer_dirs(images_dir: Path, masks_dir: Path):
    """Interactive viewer for T1s and masks stored in two separate flat directories."""
    images_dir = Path(images_dir)
    masks_dir = Path(masks_dir)
    pairs = _pairs_dirs(images_dir, masks_dir)
    ...
    status = W.HTML(f"<b>Viewer</b> — cases: {len(keys)}")
        ...
        with out:
            ...
            with plt.ioff():
                if same_shape:
                    ...
                    if cb_edges.value:
                        ax.contour(_edges2d(mask2d).T, levels=[0.5], linewidths=0.8, colors="r")
                    else:
                        ax.imshow(
                            np.ma.masked_where(~mask2d.T, mask2d.T),
                            cmap="jet", alpha=float(sl_alpha.value),
                    ...
                    axes[1].set_title("Mask slice"); axes[1].axis("off")
                plt.tight_layout()
                display(fig)
                plt.close(fig)
            ...
            if not same_shape or not same_affine:
                print("⚠️ Shapes or affines differ; mask shown separately.")
...
__all__ = ["show_viewer", "show_viewer_dirs"]
