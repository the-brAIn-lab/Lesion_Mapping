#!/bin/bash
#SBATCH --job-name=stroke_batchwise
...
echo "🚀 Stroke Segmentation with Batch-wise Data Loading"
echo "=================================================="

cd /mnt/beegfs/hellgate/home/rb194958e/stroke_segmentation_sota

# Setup environment (proven working)
...
# Create the batch-wise training script
cat > batch_wise_train.py << 'PYEOF'
#!/usr/bin/env python3
"""
...
echo "🎯 Starting batch-wise training..."
echo "This loads only 2 samples at a time - prevents OOM"

python batch_wise_train.py

echo "Training completed: $(date)"
EOF

# Submit the batch-wise training job
chmod +x scripts/batch_wise_training.sh
sbatch scripts/batch_wise_training.sh
```

## 🔧 **Key Difference - Batch-wise Loading**

**Previous Issue**: Loading all 655 volumes into memory at once  
**Solution**: `tf.keras.utils.Sequence` generator that loads only **2 samples at a time**

**Memory Usage**:
- **Previous**: 655 × (192×224×176×4 bytes) = **~70GB**
- **Now**: 2 × (192×224×176×4 bytes) = **~220MB per batch**

This keeps the **full model architecture** (32→64→128→256 filters) but manages memory properly by loading data incrementally!

Submit this and it should train the full-size model without OOM errors! 🎉
