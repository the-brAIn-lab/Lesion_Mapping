...
class CompletelyFixedSAM2Attention(layers.Layer):
    ...
    def call(self, x, training=None):
        ...
        # Memory-safe attention computation with consistent dtypes
        attention_scores = tf.matmul(queries_flat, keys_flat, transpose_b=True)
        
        # 🔧 DTYPE FIX: Ensure consistent precision for mixed precision training
        scale_factor = tf.sqrt(tf.cast(channels // 8, attention_scores.dtype))
...
def boundary_aware_loss(y_true, y_pred, smooth=1e-6):
    """Boundary-aware loss for medical segmentation with consistent dtypes"""
    y_true = tf.cast(y_true, y_pred.dtype)  # Match input dtype
    
    # Simple boundary detection using gradients
    sobel_x = tf.constant([[[[-1, 0, 1]]]], dtype=y_pred.dtype)
    sobel_y = tf.constant([[[[-1], [0], [1]]]], dtype=y_pred.dtype)
...
def smart_sota_loss_2025(y_true, y_pred, smooth=1e-6):
    """Ultimate 2025 loss function with consistent dtypes"""
    ...
    # Focal loss
    alpha, gamma = tf.cast(0.25, y_pred.dtype), tf.cast(2.0, y_pred.dtype)
    ...
    # Tversky loss
    alpha_tversky, beta_tversky = tf.cast(0.3, y_pred.dtype), tf.cast(0.7, y_pred.dtype)
    tp = tf.reduce_sum(y_true * y_pred)
    fp = tf.reduce_sum((1 - y_true) * y_pred)
    fn = tf.reduce_sum(y_true * (1 - y_pred))
    tversky_loss = 1 - (tp + smooth) / (tp + alpha_tversky * fp + beta_tversky * fn + smooth)
    
    return tf.cast(0.35, y_pred.dtype) * dice_loss + tf.cast(0.25, y_pred.dtype) * focal_loss + tf.cast(0.25, y_pred.dtype) * boundary_loss + tf.cast(0.15, y_pred.dtype) * tversky_loss
...
def dice_coefficient(y_true, y_pred, smooth=1e-6):
    y_true_f = tf.keras.backend.flatten(tf.cast(y_true, y_pred.dtype))
    y_pred_f = tf.keras.backend.flatten(y_pred)
    smooth = tf.cast(smooth, y_pred.dtype)
    intersection = tf.reduce_sum(y_true_f * y_pred_f)
...
def binary_dice_coefficient(y_true, y_pred, smooth=1e-6):
    ...
    y_pred_binary = tf.cast(y_pred > 0.5, y_pred.dtype)
    smooth = tf.cast(smooth, y_pred.dtype)
    y_true_f = tf.keras.backend.flatten(y_true)
