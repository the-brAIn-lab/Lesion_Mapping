#!/usr/bin/env python3
"""
SMART SOTA 2025: Right-Sized Model with Cutting-Edge Optimizations - FIXED VERSION
Combines proven baseline foundation with 2025 SOTA features:
- Vision Mamba blocks (linear complexity global modeling) - DIMENSION BUG FIXED
...
🔧 FIXED: Vision Mamba dimension mismatch (192 vs 176) in _ssm_step method
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('logs/smart_sota_2025_fixed.log'),
...
class SmartSOTAConfig:
    ...
    USE_VISION_MAMBA = True           # Linear-complexity global modeling - NOW FIXED
    ...
    CALLBACKS_DIR = lambda timestamp: Path(f'callbacks/smart_sota_2025_fixed_{timestamp}')
    MODEL_SAVE_PATH = lambda timestamp: f'models/smart_sota_2025_fixed_{timestamp}.h5'
...
# ============================================================================
# 2025 SOTA COMPONENTS - FIXED Vision Mamba Implementation
# ============================================================================
...
class VisionMambaBlock(layers.Layer):
    ...
    🔧 FIXED: Dimension mismatch in _ssm_step method (192 vs 176)
    """
    def __init__(self, dim, d_state=16, d_conv=4, expand=2, **kwargs):
        super().__init__(**kwargs)
        self.dim = dim
        self.d_state = d_state  # 🔧 FIXED: Ensure d_state is stored for dimension splitting
        self.d_conv = d_conv
        self.expand = expand
        ...
        self.x_proj = layers.Dense(d_state + d_inner, use_bias=False)  # Projects to 192 (16+176)
        self.dt_proj = layers.Dense(d_inner, use_bias=True)            # Projects to 176
    ...
    def call(self, x, training=None):
        ...
        # Simplified SSM computation (stable implementation) - NOW FIXED
    ...
    def _ssm_step(self, x):
        """
        🔧 FIXED SSM computation with proper dimension handling
        
        Original bug: x_proj (192 dims) * dt (176 dims) → dimension mismatch
        Fix: Split x_proj into state (16) + inner (176), use inner * dt
        """
        # Project to state space
        x_proj_full = self.x_proj(x)     # Shape: [B, seq_len, d_state + d_inner] = 192
        dt = self.dt_proj(x)             # Shape: [B, seq_len, d_inner] = 176
        dt = tf.nn.softplus(dt)          # Ensure positive
        
        # 🔧 FIX: Split x_proj into state and inner components
        x_state = x_proj_full[:, :, :self.d_state]      # [B, seq_len, 16] - state component
        x_inner = x_proj_full[:, :, self.d_state:]      # [B, seq_len, 176] - inner component
        
        # ✅ NOW WORKS: Use x_inner (176) * dt (176) instead of x_proj_full (192) * dt (176)
...
class SAM2InspiredAttention(layers.Layer):
    ...
    def call(self, x, training=None):
        ...
        attention_scores = attention_scores / tf.sqrt(tf.cast(channels // 8, tf.float16))
...
class AdvancedConvBlock(layers.Layer):
    ...
    BatchNorm + GELU + SE attention (proven components) + FIXED Mamba
    """
    def __init__(self, filters, use_mamba=False, use_sam2_attention=False, **kwargs):
        ...
        if use_mamba:
            self.mamba_block = VisionMambaBlock(filters)  # 🔧 NOW FIXED
    ...
    def call(self, x, training=None):
        ...
        # 2025 SOTA enhancements - NOW WITH FIXED MAMBA
        if self.use_mamba:
            x = self.mamba_block(x, training=training)  # ✅ Should work now
...
def build_smart_sota_2025_model(input_shape=(192, 224, 176, 1), base_filters=22):
    ...
    🔧 FIXED: Vision Mamba dimension issues resolved
    """
    ...
    # Stage 3: 88 filters - Enable FIXED Mamba
    conv3 = AdvancedConvBlock(
        base_filters * 4,
        use_mamba=True,  # ✅ Enable global modeling - NOW FIXED
    ...
    model = tf.keras.Model(inputs=inputs, outputs=output, name='SmartSOTA2025_Fixed')
    
    return model

# ============================================================================
# 2025 SOTA LOSS FUNCTIONS (unchanged)
# ============================================================================
...
# ============================================================================
# 2025 ADVANCED DATA AUGMENTATION (unchanged)
# ============================================================================
...
# ============================================================================
# METRICS AND CALLBACKS (unchanged)
# ============================================================================
...
def train_smart_sota_2025():
    """
    Train the Smart SOTA 2025 model - FIXED VERSION
    Applies all lessons learned from previous troubleshooting + Mamba fix
    """
    logger.info("🔧 SMART SOTA 2025 TRAINING - FIXED VERSION")
    logger.info("=" * 80)
    ...
    logger.info("✅ FIX APPLIED: Vision Mamba dimension mismatch resolved")
    logger.info("🔧 Fixed: x_proj (192) vs dt (176) → split x_proj into components")
    logger.info("=" * 80)
    ...
    with strategy.scope():
        logger.info("Building Smart SOTA 2025 model - FIXED VERSION...")
    ...
    # Model summary
    logger.info("\n" + "=" * 80)
    logger.info("SMART SOTA 2025 ARCHITECTURE SUMMARY - FIXED")
    logger.info("=" * 80)
    logger.info(f"📊 Total Parameters: {param_count:,}")
    logger.info(f"🎯 Target Performance: 68-75% validation Dice")
    logger.info(f"🔧 Key Features:")
    logger.info(f"   ✅ Vision Mamba blocks (FIXED - linear complexity)")
    ...
    logger.info(f"🔧 MAMBA FIX:")
    logger.info(f"   ✅ Split x_proj into state (16) + inner (176) components")
    logger.info(f"   ✅ Use inner component for multiplication with dt")
    logger.info(f"   ✅ Dimension compatibility: 176 × 176 ✓")
    logger.info("=" * 80)
    ...
    # Train the Smart SOTA 2025 model - FIXED VERSION
    try:
        logger.info("🔥 Starting Smart SOTA 2025 training - FIXED VERSION...")
        history = model.fit(
            train_generator,
        ...
        logger.info("=" * 80)
        logger.info("🏆 SMART SOTA 2025 TRAINING RESULTS - FIXED")
        logger.info("=" * 80)
...
if __name__ == "__main__":
    ...
    try:
        ...
        if success:
            ...
            logger.info("🔧 Vision Mamba dimension fix applied - 2025 optimizations working!")
        else:
