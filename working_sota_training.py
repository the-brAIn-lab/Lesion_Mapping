...
Uses all 600 samples, increased regularization, and mixed precision.
"""
...
class Config:
    ...
    BATCH_SIZE = 1
    EPOCHS = 50
    BASE_FILTERS = 8  # Fits 24GB VRAM
    VALIDATION_SPLIT = 0  # Use all 600 samples for training
    LEARNING_RATE = 1e-4
...
def create_callbacks(callbacks_dir):
    ...
    return [
        tf.keras.callbacks.ModelCheckpoint(
            ...
            monitor='dice_coefficient',  # Monitor training Dice
            mode='max',
            save_best_only=True,
            verbose=1
        ),
...
def train_model():
    ...
    train_size = len(image_ids)  # Use all 600 samples
